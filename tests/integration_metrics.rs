...
mod tests {
    ...
    /// - cache eviction (fill cache beyond max_size_bytes)
    /// - upstream error (GET non-existent key)
    async fn generate_traffic(proxy: &TestProxy, minio_port: u16) {
        ...
        for i in 0..10 {
            let key = format!("evict-{i}.txt");
            direct
                .put_object()
                .bucket(&bucket)
                .key(&key)
                .body(ByteStream::from(Bytes::from(format!(
                    "eviction-payload-{i:04}"
                ))))
                .send()
                .await
                .unwrap();
    ...
    async fn all_metrics_exported_with_correct_prometheus_names() {
        ...
        let config = s3_cache::Config {
            ...
            cache_max_size_bytes: 120,
            cache_max_object_size_bytes: 100,
        ...
        let body = poll_prometheus(&prom_url, timeout, |b| {
            b.contains("s3_cache_hit_bytes_total")
                && b.contains("s3_cache_miss_bytes_total")
                && b.contains("s3_cache_eviction_bytes_total")
                && b.contains("upstream_error_total")
        })
        .await;
        ...
        assert!(
            body.contains("s3_cache_eviction_bytes_total"),
            "missing counter: s3_cache_eviction_bytes_total"
        );
        assert!(
            body.contains("s3_cache_eviction_bytes_histogram_bucket"),
            "missing histogram: s3_cache_eviction_bytes_histogram"
        );
        assert!(
            body.contains("s3_cache_eviction_age_histogram_seconds_bucket"),
            "missing histogram: s3_cache_eviction_age_histogram_seconds"
        );

        assert!(
            body.contains("upstream_error_total"),
            "missing counter: upstream_error_total"
        );
    }
}
